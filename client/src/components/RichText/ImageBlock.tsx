...
export const ImageBlock = ({
  ...
  function getUpdateAlignment(alignment: string) {
    return () => {
      const currentSelection = editorState.getSelection()
      const entityKey = block.getEntityAt(0)
      contentState.mergeEntityData(entityKey, {
    ...
    <span>
      ...
      {showPopover && (
        <div contentEditable={false} className="popover">
          ...
          <span className="divider"></span>
          <button onClick={getUpdateAlignment('left')}>L</button>
          <span className="divider"></span>
          <button onClick={getUpdateAlignment('center')}>C</button>
          <span className="divider"></span>
          <button onClick={getUpdateAlignment('right')}>R</button>
          <span className="divider"></span>
